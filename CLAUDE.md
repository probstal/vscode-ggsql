...
├── scripts/
│   └── update-vendor.sh      Re-vendor grammar + datasets, rebuild the grammar wasm
└── vendor/                   Pinned to the ggsql-wasm version (ref in each UPSTREAM file)
    ├── tree-sitter-ggsql/    Vendored ggsql grammar + built tree-sitter-ggsql.wasm
    └── ggsql-datasets/       Builtin datasets (ggsql:penguins & co.) as parquet
```
...
When bumping the `ggsql-wasm` dependency, re-vendor the split grammar and the builtin datasets to the matching upstream tag: `scripts/update-vendor.sh v<version>` (needs the tree-sitter CLI devDependency; downloads wasi-sdk to `~/.cache/tree-sitter` on first wasm build) and commit the `vendor/` changes including `tree-sitter-ggsql.wasm`.
